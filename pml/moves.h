...
STRUCT_DECLARE(MoveData)

ENUM_DECLARE(MoveID)
ENUM_DECLARE(MoveField)
ENUM_DECLARE(MoveQuality)

C_DECL_BEGIN

u32 PML_MoveGetParam(int wazaId, MoveField dataId);
u32 PML_MoveGetParamCore(MoveData *pMoveData, MoveField move_field);
int PML_MoveGetStatChangeStat(int wazaId);
int PML_MoveGetStatChangeStage(int wazaId, int statIdx, int *dest);
int PML_MoveGetMaxPP(int wazaId, unsigned int ppUpStage);
u32 PML_MoveGetQuality(int wazaId);
u8 PML_MoveGetType(int wazaId);
u16 PML_MoveGetBasePower(int wazaId);
u32 PML_MoveGetCategory(int wazaId);
bool PML_MoveIsAlwaysHit(int wazaId);
bool PML_MoveIsAlwaysCrit(int wazaId);
bool PML_MoveIsDamaging(int wazaId);
void PML_MoveInitDataCache(int cacheSize, HeapID heapId);
void PML_MoveFreeDataCache();
bool PML_MoveIsDataCachePresent();
b32 PML_MoveIsHM(u16 moveNum);
...
{
  QUALITY_NONE = 0x0,
  QUALITY_STATUS_INFLICTING = 0x1,
  QUALITY_TARGET_STAT_CHANGING = 0x2,
  QUALITY_HEALING = 0x3,
  QUALITY_STATUS_INFLICTING_CHANCE = 0x4,
  QUALITY_RAISE_TARGET_STAT = 0x5,
  QUALITY_LOWER_TARGET_STAT = 0x6,
  QUALITY_RAISE_USER_STAT = 0x7,
  QUALITY_DRAINING = 0x8,
  QUALITY_OHKO = 0x9,
  QUALITY_WEATHER = 0xA,
  QUALITY_SAFEGUARD = 0xB,
  QUALITY_FORCE_SWITCH = 0xC,
  QUALITY_UNIQUE_EFFECT = 0xD,
};
